...
function onTicker() {
    ...
    if (dir == 1) {
        ...
        // }
        openAction("buy", currentPrice - 4, amount)
    ...
    if (dir == -1) {
        ...
        // }
        openAction("sell", currentPrice + 4, amount)
    }

    if (dir == 2) {
        Log("D止损")
        cancleCoverOrders(ORDER_TYPE_SELL)
        closeBuyAction(currentPrice + 4, asset.buyAmount)
...
function getDirection() {
    ...
    if (d > 0) {
        ...
        if ((nowTime - asset.buytime) < 1000 * 62) {
            return 0;
        }
        asset.buytime = nowTime;
        var records2 = exchange.GetRecords(PERIOD_M1) //4小时
        if (!records2 || records2.length < 10) {
            Log('请求问题,休眠3s...')
            Sleep(1000 * 3)
            return 0;
        }
        var oneCCI = getCCIValue2(records2)
        ccigy = oneCCI;
        if (asset.buyAmount > 0 && asset.openBuyPrice - currentPrice > 5 && oneCCI < -100) {
            return 1;
        }
        if (oneCCI < -100 && asset.buyAmount == 0) {
            return 1;
        }
    ...
    if (d < 0) {
        ...
        var nowTime = Date.parse(new Date());
        if ((nowTime - asset.selltime) < 1000 * 62) {
            return 0;
        }
        asset.selltime = nowTime;
        var records2 = exchange.GetRecords(PERIOD_M1) //4小时
        if (!records2 || records2.length < 10) {
            Log('请求问题,休眠3s...')
            Sleep(1000 * 3)
        ...
        var oneCCI = getCCIValue2(records2)
        ccigy = oneCCI;
        if (asset.sellAmount > 0 && currentPrice - asset.openSellPrice > 5 && oneCCI > 100) {
            return -1;
        }
        if (oneCCI > 100 && asset.sellAmount == 0) {
            return -1;
        }
...
function calculateStopProfile() {
    ...
    if (asset.buyAmount > 0) {
        ...
        if (oneCCI > 80) {
            var nowTime = Date.parse(new Date());
            if ((nowTime - asset.coverbuytime) < 1000 * 62) {
                return 0;
            }
            asset.coverbuytime = nowTime;
            cancleCoverOrders(ORDER_TYPE_SELL)
                             cancleaddAction(ORDER_TYPE_SELL)

            if (asset.buyAmount - 0.02 <= 0) {
                closeBuyAction(currentPrice + 4 , asset.buyAmount)
                return
            }

            closeBuyAction(currentPrice + 4, 0.02)
    ...
    if (asset.sellAmount > 0) {
        ...
        if (oneCCI < -80) {
            var nowTime = Date.parse(new Date());
            if ((nowTime - asset.coverselltime) < 1000 * 62) {
                return 0;
            }
            asset.coverselltime = nowTime;
            cancleCoverOrders(ORDER_TYPE_BUY)
            cancleaddAction(ORDER_TYPE_BUY)

            if (asset.sellAmount - 0.02 <= 0) {
                closeSellAction(currentPrice - 4, asset.sellAmount)
                return
            }

            closeSellAction(currentPrice - 4, 0.02)
    ...
    if (asset.buyAmount > 0) {
        ...
        if (profile >= initBuyProfile) {
            ...
            if (stopBuyOne) {
                ...
                var sells = 0.02;
                if (asset.buyAmount - 0.02 <= 0) {
                    sells = asset.buyAmount
                }
                closeBuyAction(currentPrice - 4, sells)
    ...
    if (asset.sellAmount > 0) {
        ...
        if (profile >= initSellProfile) {
            ...
            if (stopSellOne) {
                ...
                var sells = 0.02;
                if (asset.sellAmount - 0.02 <= 0) {
                    sells = asset.sellAmount
                }
                closeSellAction(currentPrice + 4, sells)
...
function checkOpenBuyOrSellAmount(isBuy) {

    var currentPrice = GetTicker().Last;
    if (asset.buyAmount > 0) {
        if (asset.openBuyPrice - currentPrice / asset.openBuyPrice) {
