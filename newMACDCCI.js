        ...
        for (var i = 0; i < position.length; i++) {
            if (position[i].Type == 0) {
                ...
                if (position[i].Amount != position[i].FrozenAmount) {
                    ...
                    startBuyGrids(parseFloat(position[i].Price + 3 * atrValue),position[i].Amount)
                }
                ...
                if (position[i].Amount != position[i].FrozenAmount) {
                    startSellGrids(parseFloat(position[i].Price - 3 * atrValue),position[i].Amount);
                }
...
function main() {
   ...
   exchange.IO("currency", "BSV_USDT")
    ...
    while (true) {
        ...
        if (sum >= 4 && asset.buyAmount < 2) {
            
            cancleaddAction(ORDER_TYPE_BUY, true)
            cancleaddAction(ORDER_TYPE_SELL, true)
            openAction("buy", 4)
            asset.lowPrice = TA.Lowest(records, 14, 'Low');;
        ...
        if (sum <= -4 && asset.sellAmount < 2) {
            ...
            cancleaddAction(ORDER_TYPE_BUY, true)
            openAction("sell", 4)
            asset.highPrice = TA.Highest(records, 14, 'High');
