    ...
    if (dir == 1) {
       ...
       // if (asset.highPrice <= currentPrice * 0.99) {
        ...
        if ( asset.buyAmount > 0) {
            amount = 2 * amount;
           if (asset.openBuyPrice - currentPrice < 100) {
               return
           }
        }
    ...
    if (dir == -1) {
       ...
       // if (asset.lowPrice >= currentPrice * 0.99) {
        ...
        if (asset.sellAmount > 0) {
            amount = 2 * amount
           if (currentPrice - asset.openSellPrice  < 100) {
               return
           }
        }
...
function getDirection() {
    //getHighLow(true)
    var records = exchange.GetRecords(PERIOD_M1) //4小时
...
function calculateStopLoss() {
    ...
    if (asset.stopLossDPrice > asset.openBuyPrice) {
        if (asset.buyAmount > 0) {
            Log('突破最低价止损D')
            var currentPrice = GetTicker().Last;
            cancleCoverOrders(ORDER_TYPE_SELL)
    ...
    asset.stopLossKPrice = TA.Lowest(records, 120, 'High');//止损价格
     if (asset.stopLossKPrice > asset.openSellPrice) {
        if (asset.sellAmount > 0) {
            Log('突破最高价止损K')
            var currentPrice = GetTicker().Last;
            cancleCoverOrders(ORDER_TYPE_BUY)
